...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Movement : MovementBehaviour
    {
        ...
        [SerializeField]
        private float flyAcceleration = 10f;
        ...
        [Tooltip("Maximum flying time.")]
        [SerializeField]
        private float maximumFlyingTime = 15f;

        [SerializeField]
        private float flyForce = 0.5f;

        [Tooltip("After this value time since last flying, gravity will apply normally to character")]
        [SerializeField]
        private float delayTimeToStartFalling = 1f;
        #endregion
        ...
        /// <summary>
        /// Stores the Time.time value when the character last flew.
        /// </summary>
        private float lastFlyTime;

        /// <summary>
        /// If remainingTime = 0, normal gravity will apply.
        /// </summary>
        [SerializeField] private float flyingTimeRemaining;
        #endregion
        ...
        protected override void Awake()
        {
            ...
            //Fill fly bar when claming jetpack.
            this.RegisterListener(EventID.OnClaimJetpack, (param) =>
            {
                flyingTimeRemaining = maximumFlyingTime;
                this.PostEvent(EventID.OnChangeFlyingTime, flyingTimeRemaining / maximumFlyingTime);
            });
        ...
        protected override void Start()
        {
            ...
            //Fill the character fly bar at start.
            flyingTimeRemaining = maximumFlyingTime;
        }
        ...
        protected override void Update()
        {
            ...
            //Fly.
            //Modify gravity due to last fly time.
            if (Time.time - lastFlyTime < delayTimeToStartFalling)
            {
                gravity = 0;
            }
            else
        ...
        private void MoveCharacter()
        {
            ...
            if (frameInput.y > 0.8f && !isFlying)
            {
                ((Character)playerCharacter).HoldingButtonRun = true;
            }
            else
            {
                ((Character)playerCharacter).HoldingButtonRun = false;
            }
            ...
            if (isGrounded == false)
            {
                ...
                {
                    desiredDirection *= speedFlying;
                    velocity = Vector3.Lerp(velocity, new Vector3(desiredDirection.x, velocity.y, desiredDirection.z), Time.deltaTime * flyAcceleration);
                }
            }
        ...
        {
            if (flyingTimeRemaining > 0)
            {
                jumping = true;
                
                velocity = new Vector3(velocity.x, Mathf.Sqrt(2.0f * flyForce * jumpGravity), velocity.z);

                flyingTimeRemaining -= Time.deltaTime;
                this.PostEvent(EventID.OnChangeFlyingTime, flyingTimeRemaining / maximumFlyingTime);
        ...
        public override void SetFlyingState(bool state)
        {
            isFlying = state;
        }
