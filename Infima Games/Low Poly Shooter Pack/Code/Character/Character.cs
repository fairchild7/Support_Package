...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		protected override void Awake()
		{
			...
			//Register change weapon event.
			this.RegisterListener(EventID.OnChangeWeapon, (param) =>
			{
				if (gameObject.activeInHierarchy)
					OnTryChangeWeapon((WeaponType)param);
			});

			this.RegisterListener(EventID.OnClaimWeapon, (param) =>
			{
				if (gameObject.activeInHierarchy)
				{
					if (equippedWeapon.GetWeaponType() != (WeaponType)param)
					{
						OnTryChangeAndFillWeapon((WeaponType)param);
						//equippedWeapon.FillAmmunition(0);
					}
					else
					{
            ...
            this.RegisterListener(EventID.OnPlayerReviveAds, (param) =>
            {
                reloading = false;
            });
        }
		...
		private IEnumerator Equip(int index = 0)
		{
            ...
            this.PostEvent(EventID.UpdateMainGunBullet, equippedWeapon.GetAmmunitionCurrent().ToString());

            WeaponStat weaponStat = DataManager.Ins.WeaponData.GetWeaponStat(equippedWeapon.GetWeaponType());
			if (weaponStat != null)
			{
				Debug.Log("Normal change");
				this.PostEvent(EventID.ActiveTextGunBullet, weaponStat.isMainGun);
				this.PostEvent(EventID.ChangeMainGunImage, weaponStat.sprite);
                UIManager.Ins.GetUI<UIGameplay>().MainGunHighlight();

                if (equippedWeapon.GetWeaponType() == WeaponType.Crowbar)
				{
					this.PostEvent(EventID.SetMelee, true);
                    this.PostEvent(EventID.ActiveTextGunBullet, false);
                }
				else
				{
                    this.PostEvent(EventID.SetMelee, false);
                }
			}
			else
			{
				this.PostEvent(EventID.ChangeMainGunToCachedImage);
                this.PostEvent(EventID.ActiveTextGunBullet, false);
                UIManager.Ins.GetUI<UIGameplay>().MainGunHighlightOff();
			}
        }

        private IEnumerator EquipAndFill(int index = 0)
        {
            //Only if we're not holstered, holster. If we are already, we don't need to wait.
            if (!holstered)
            {
        ...
        private void RefreshWeaponSetup()
		{
			...
			UIManager.Ins.GetUI<UIGameplay>().EnableMoveButton(equippedWeapon.GetWeaponType() == WeaponType.SpawnGun);
        }
		...
		/// Fly
		/// </summary>
		public void OnTryFly(InputAction.CallbackContext context)
		{
			switch (context.phase)
			{
				case InputActionPhase.Started:
					...
					movementBehaviour.SetFlyingState(true);
					break;
				case InputActionPhase.Performed:
					break;
				case InputActionPhase.Canceled:
					movementBehaviour.SetFlyingState(false);
					break;
			}
		}
		...
		public void OnTryChangeWeapon(WeaponType weaponType)
		{
			int indexChange = inventory.GetWeaponIndex(weaponType);
			int indexCurrent = inventory.GetEquippedIndex();

			if (CanChangeWeapon() && (indexCurrent != indexChange))
			{
				StartCoroutine(nameof(Equip), indexChange);

				Garbage.GarbageFunction();
				characterMesh.enabled = !(weaponType == WeaponType.Crowbar);
			}
		}

		public void OnTryChangeAndFillWeapon(WeaponType weaponType)
		{
            int indexChange = inventory.GetWeaponIndex(weaponType);
            int indexCurrent = inventory.GetEquippedIndex();

            if (CanChangeWeapon() && (indexCurrent != indexChange))
                StartCoroutine(nameof(EquipAndFill), indexChange);
        }
		...
		{
			get => holdingButtonRun;
			set => holdingButtonRun = value;
		}
		#endregion
