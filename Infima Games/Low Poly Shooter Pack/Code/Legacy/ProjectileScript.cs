...
namespace InfimaGames.LowPolyShooterPack.Legacy
{
	public class ProjectileScript : MonoBehaviour
	{
		...
		[SerializeField] List<Collider> owners = new List<Collider>();
		...
		public void RegisterOwner(List<Collider> ownerCollider)
		{
			owners = ownerCollider;

			foreach (Collider coll in owners)
			{
		...
		private IEnumerator ExplodeSelf()
		{
			...
			if (!hasCollided)
			{
				...
				{
					Debug.Log("instantiate explosion 1");
		...
		private void OnCollisionEnter(Collision collision)
		{
			...
			if (owners.Contains(collision.collider))
				return;
			...
			if (explosionPrefab != null)
			{
				Debug.Log("instantiate explosion 2");
   ...
   //                 Instantiate(explosionPrefab, collision.contacts[0].point,
			...
			*/
            ...
            Debug.Log("Exploded hit " + collision.transform.gameObject.name + " and exploded");

			List<ITargetable> damaged = new List<ITargetable>();
			...
			foreach (Collider hit in colliders)
			{
				...
				*/
                ...
                Debug.Log(hit.gameObject.name + " hit by exploded projectile!");
